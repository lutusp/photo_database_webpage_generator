...
The Web page includes a text entry that allows the user to enter a search string. As the user enters a search, the displayed images change to those whose descriptions matrch the search entry. This search feature allows the user to find specific images in large image archives.
...
This second Web page has what may be a non-obvious advantage – if the user creates a USB stick for portable presentations, the relative-address Web page will work from any system the USB stick is connected to.


Here are the project’s requirements, easily changed by editing the Python source:
...
The user may substitute another LLM of similar ability, but the LLM must be able to read graphic images.
...
Four example data records are included to familiarize users with the procedure. Be sure to remove these examples, or avoid using their names in the main program loop.

It’s a good idea to start with a small set of graphic images. This program can create huge databases and Web pages, but this can lead to problems. I have successfully created Web pages that provide access to over 8,000 images, but:

\* Such a large Web page can be difficult to use.

\* Only certain browsers will tolerate such large image sets, in fact, in my experience, the only browser that tolerates large image sets is Firefox. It’s best to avoid huge image sets within a single Web page.

In practice, with a large image collection, the program does the following:
